/*
 * mln.c - a part of driver for Yamaha MLN3 board module.
 *
...
MODULE_DESCRIPTION("Driver for Yamaha MLN3 board module");
...
{
	//snd_mln_transaction_unregister(mln);
...
static void do_registration(struct work_struct *work)
{
	...
	snd_mln_proc_init(mln);

	/*
	err = snd_mln_transaction_register(mln);
	if (err < 0)
		goto error;
		*/
...
{
	struct snd_mln *mln;
	struct snd_mln_protocol *protocol;

	protocol = (struct snd_mln_protocol *)entry->driver_data;
	if (!protocol)
		return -ENODEV;
...
static void snd_mln_update(struct fw_unit *unit)
{
	...
	if (!mln->registered)
		...
		snd_mln_transaction_reregister(mln);
}
...
static const struct ieee1394_device_id snd_mln_id_table[] = {
	...
	/* Yamaha n series (n8/n12). */
	{
		...
		.version	= 0x000300,
		.model_id	= 0x100010,
		.driver_data	= (kernel_ulong_t)&snd_mln_protocol_v3,
	},
	/* Yamaha MR816 series (Steinberg MR816x/Steinberg MR816csx) */
	{
		.match_flags	= IEEE1394_MATCH_VENDOR_ID |
				  IEEE1394_MATCH_SPECIFIER_ID |
				  IEEE1394_MATCH_VERSION |
