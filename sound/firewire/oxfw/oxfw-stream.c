		      ...
		      u8 **formats)
{
	...
	eid = 0;
	err = avc_stream_parse_format(buf, &formation);
	if (err < 0)
		goto end;
	...
	for (i = 0; i < ARRAY_SIZE(avc_stream_rate_table); i++) {
		if (formation.rate == avc_stream_rate_table[i])
			continue;

		err = avc_general_inquiry_sig_fmt(oxfw->unit,
						  avc_stream_rate_table[i],
						  dir, pid);
		if (err < 0)
			continue;
		...
		formats[eid][2] = avc_stream_rate_codes[i];
	}
			       ...
			       unsigned short pid)
{
	...
	struct avc_stream_formation dummy;
	int err;
	...
	while (eid < SND_OXFW_STREAM_FORMAT_ENTRIES) {
		...
		/* parse and set stream format */
		err = avc_stream_parse_format(buf, &dummy);
		if (err < 0)
			break;
